...
Exploratory only, not meant to ship. Each probe is isolated in its own activity
with its own try/except, so one failure doesn't stop the others — the workflow
reports every raw result or error back in the chat so we can see empirically
what's actually available in this environment, instead of guessing from docs.
...
this worker also registers hello-world/hello-chat/rag-qa, a registration
failure here risks breaking `make start-worker` for all of them. This file
tests the safe, catchable failure mode only.
"""
